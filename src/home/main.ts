...
const PAD = 44;
const CONTENT_WIDTH = CANVAS_WIDTH - PAD * 2;
const COLS = 3;
const GAP = 22;

const CARD_WIDTH = Math.floor((CONTENT_WIDTH - GAP * (COLS - 1)) / COLS);
...
const CARD_ROW = {
  cover: COVER_INSET,
  title: COVER_INSET + COVER_HEIGHT + 8,
  tagline: COVER_INSET + COVER_HEIGHT + 38,
  tags: COVER_INSET + COVER_HEIGHT + 64,
  actions: COVER_INSET + COVER_HEIGHT + 96,
};
const CARD_TAG_HEIGHT = 22;
const CARD_BUTTON = { width: 112, height: 38 };
const CARD_HEIGHT = CARD_ROW.actions + CARD_BUTTON.height + 15;
...
const STAT_WIDTH = Math.floor((CONTENT_WIDTH - GAP * 2) / 3);
 ...
 */
function layoutHome(): HomeLayout {
  const statsTop = HERO.statsTop;
  const heroSeparatorTop = statsTop + HERO.statsHeight + HERO.separatorGap;

  // 第一个分区从分隔线往下 `SECTION_GAP` 开始
  let cursor = heroSeparatorTop + 1 + SECTION_GAP;

  const slides = featuredSlides();
  let featured: HomeLayout['featured'] = null;
  if (slides.length >= 2) {
    const headerTop = cursor;
    const carouselTop = headerTop + SECTION_HEADER_HEIGHT;
    featured = { headerTop, carouselTop, carouselHeight: FEATURED.height, slides };
    cursor = carouselTop + featured.carouselHeight + SECTION_GAP;
  }

  const sections: SectionHeader[] = GROUPS.map((group) => {
    const headerTop = cursor;
    const gridTop = headerTop + SECTION_HEADER_HEIGHT;
    const rows = Math.ceil(group.items.length / COLS);
    cursor = gridTop + rows * CARD_HEIGHT + (rows - 1) * GAP + SECTION_GAP;
    return { kind: group.kind, label: group.label, blurb: group.blurb, headerTop, gridTop, rows };
  });

  const noteTop = cursor;
  const footerTop = noteTop + NOTE_HEIGHT + FOOTER_GAP;
  const footerHeight = measureFooterHeight(CONTENT_WIDTH);

  return {
    height: Math.max(MIN_CANVAS_HEIGHT, footerTop + footerHeight + PAD),
    heroSeparatorTop,
    statsTop,
    statsHeight: HERO.statsHeight,
    statWidth: STAT_WIDTH,
    featured,
    sections,
    noteTop,
    footerTop,
...
const kindBadgeText = (kind: PageKind): string => (kind === 'machine' ? '整机' : '小游戏');

/** 一条横向标签的尺寸（`ICETag` 的默认高度是 24，这里压到 22 让卡片更紧凑）。 */
function tagSize(text: string): { width: number; height: number } {
  return { width: fitControlWidth(text, TAG_FONT_SIZE, TAG_FONT_WEIGHT), height: CARD_TAG_HEIGHT };
}

/** 类型徽标尺寸（`ICEBadge` 的 `variant: 'soft'`）。 */
function badgeSizeFor(text: string): { width: number; height: number } {
  return { width: fitControlWidth(text, TAG_FONT_SIZE, BADGE_FONT_WEIGHT), height: 22 };
}
 ...
 */
function trailLine(left: number, top: number): any {
  return ice.addChild(
    new ICESeparator({
      interactive: false,
      left,
      top,
      width: Math.max(0, PAD + CONTENT_WIDTH - left),
      height: 1,
    }),
  );
}
 ...
 */
{
  const glowPanel = new ICEPanel({
    ...
    top: 0,
    width: CANVAS_WIDTH,
    height: layout.heroSeparatorTop,
    radius: 0,
    style: { fillStyle: 'rgba(0,0,0,0)', strokeStyle: 'rgba(0,0,0,0)', lineWidth: 0, shadowBlur: 0 },
  });
  ice.addChild(glowPanel, false);

  const primaryGlow = ice.createRadialGradient(PAD + 250, PAD + 20, 10, PAD + 250, PAD + 20, 560);
  ...
  const cyanGlow = ice.createRadialGradient(CANVAS_WIDTH * 0.82, PAD + 60, 10, CANVAS_WIDTH * 0.82, PAD + 60, 420);
...
const badgeGradient = ice.createLinearGradient(HERO.top, HERO.top + 2, HERO.top + HERO.badgeSize, HERO.top + 2 + HERO.badgeSize);
...
const TITLE_LEFT = PAD + HERO.badgeSize + 18;
...
ice.addChild(
  new ICELabel({
    ...
    left: TITLE_LEFT,
    top: HERO.top,
    width: CONTENT_WIDTH - (TITLE_LEFT - PAD) - 380,
    height: 46,
    verticalAlign: 'middle',
    text: '画布游戏厅',
    style: { fontSize: 34, fontWeight: '700', fillStyle: theme.colors.text },
  }),
);
  ...
  cards.forEach((card, index) => {
    ice.addChild(
      new ICEStatCard({
        ...
        interactive: false,
        left: PAD + index * (layout.statWidth + GAP),
        top: layout.statsTop,
        ...
        trend: card.trend,
        trendType: 'info',
        style: { fillStyle: theme.colors.surface, strokeStyle: theme.colors.border, shadowBlur: 0 },
      }),
    );
  });
}
...
/* hero 与正文之间的分隔线。 */
trailLine(PAD, layout.heroSeparatorTop);
 ...
 * 换主题时不会漏掉这一处。
 */
function buildSectionHeader(options: {
  icon: string;
  iconColor: string;
  label: string;
  blurb: string;
  ...
  ice.addChild(
    new ICEIcon({
      interactive: false,
      left: PAD,
      top: top + Math.round((SECTION_HEADER_HEIGHT - iconSize) / 2),
      size: iconSize,
      icon,
      color: iconColor,
    }),
  ...
  const labelLeft = PAD + iconSize + 12;
  ...
  ice.addChild(
    new ICELabel({
      ...
      left: blurbLeft,
      top,
      width: Math.max(80, PAD + CONTENT_WIDTH - blurbLeft - 40),
      height: SECTION_HEADER_HEIGHT,
      verticalAlign: 'middle',
      text: blurb,
  ...
  trailLine(lineLeft, top + Math.round(SECTION_HEADER_HEIGHT / 2));
}
 ...
 */
function buildFeatured(spec: NonNullable<HomeLayout['featured']>): ICECarousel {
  const slides = spec.slides.map((game) => buildFeaturedSlide(game, spec.carouselHeight));
   ...
   * 不会把这里设的底色 / 坐标改回去。
   */
  for (const arrow of [carousel.getPrevButton(), carousel.getNextButton()]) {
    if (arrow) {
  ...
  for (let index = 0; index < carousel.getCount(); index += 1) {
    const dot = carousel.getDotNode(index);
    if (!dot) continue;
    dot.setState({
      left: FEATURED.padLeft + index * (DOT.size + DOT.gap),
...
function buildFeaturedSlide(game: GamePage, height: number): any {
  ...
  const tint = ice.createRadialGradient(
  ...
  /* 左侧文案列 */
  const textLeft = FEATURED.padLeft;
  const textColWidth = coverLeft - textLeft - 26;

  const kindText = kindBadgeText(game.kind);
  const kindBadge = badgeSizeFor(kindText);
  slide.addChild(
    new ICEBadge({
      interactive: false,
      left: textLeft,
  ...
  const barGradient = ice.createLinearGradient(0, 0, 0, 34);
  barGradient.addColorStop(0, game.accent);
  barGradient.addColorStop(1, shade(game.accent, -0.3));
  slide.addChild(
    new ICEPanel({
      interactive: false,
      left: textLeft,
  ...
  slide.addChild(
    new ICELabel({
      ...
      verticalAlign: 'middle',
      text: game.tagline,
      style: { fontSize: 13.5, fillStyle: theme.colors.textSecondary },
    }),
    false,
  );
  ...
  const btnGradient = ice.createLinearGradient(0, 0, 138, 42);
  btnGradient.addColorStop(0, game.accent);
  btnGradient.addColorStop(1, shade(game.accent, -0.24));
...
function buildSheen(parent: any, left: number, top: number, width: number, height: number): any {
  const sheen = ice.createLinearGradient(0, 0, width, height);
...
function renderCoverPlaceholder(card: any, game: GamePage): void {
  ...
  card.addChild(
    new ICELabel({
      interactive: false,
      left: COVER_INSET,
      top: CARD_ROW.cover + Math.round(COVER_HEIGHT / 2) - 30,
      width: COVER_WIDTH,
      align: 'center',
  ...
  card.addChild(
    new ICELabel({
      interactive: false,
      left: COVER_INSET,
      top: CARD_ROW.cover + Math.round(COVER_HEIGHT / 2) + 32,
      width: COVER_WIDTH,
      align: 'center',
...
function buildCard(game: GamePage, left: number, top: number): CardNodes {
  ...
  const card = new ICECard({
    ...
    radius: CARD_RADIUS,
    title: game.title,
    paddingLeft: COVER_INSET,
    paddingTop: CARD_ROW.title,
   ...
   */
  const cardTop = ice.createLinearGradient(0, 0, CARD_WIDTH, 0);
  ...
  const sheen = buildSheen(card, COVER_INSET, CARD_ROW.cover, COVER_WIDTH, COVER_HEIGHT);
  ...
  const chipLimit = CARD_WIDTH - COVER_INSET - CARD_BUTTON.width - 14;
  for (let i = 0; i < chipSource.length; i += 1) {
    const text = chipSource[i];
    const size = tagSize(text);
  ...
  const btnLeft = CARD_WIDTH - COVER_INSET - CARD_BUTTON.width;
  const btnGradient = ice.createLinearGradient(0, 0, CARD_BUTTON.width, CARD_BUTTON.height);
  btnGradient.addColorStop(0, game.accent);
  btnGradient.addColorStop(1, shade(game.accent, -0.22));
  const button = new ICEButton({
    id: `game-enter-${game.slug}`,
    left: btnLeft,
    top: CARD_ROW.actions,
    width: CARD_BUTTON.width,
    height: CARD_BUTTON.height,
    radius: 9,
    text: '进入',
 ...
 */
const sectionAnchors = layout.sections.map((section) => ({
  key: section.kind,
  label: section.label,
  canvasY: section.headerTop,
}));

const carousel = layout.featured ? buildFeatured(layout.featured) : null;

layout.sections.forEach((section, index) => {
  const group = GROUPS[index];
  buildSectionHeader({
    icon: section.kind === 'game' ? '▶' : '■',
    iconColor: section.kind === 'game' ? theme.colors.primary : '#a370f7',
    label: section.label,
  ...
  group.items.forEach((game, itemIndex) => {
    ...
    nodes[game.slug] = buildCard(game, PAD + col * (CARD_WIDTH + GAP), section.gridTop + row * (CARD_HEIGHT + GAP));
  });
});
...
const footer = buildFooter({
  ...
  coverProgress: { done: PAGES.length - missing.length, total: PAGES.length },
});
 ...
 */
{
  const footerBottom = layout.footerTop + footer.height;
  ...
  if (footer.height !== layout.footerHeight) {
    throw new Error(
      `页脚高度与预留不一致：渲染 ${footer.height}px vs 预留 ${layout.footerHeight}px` +
   ...
   */
  if (CONTENT_WIDTH !== CANVAS_CONTENT_WIDTH) {
    throw new Error(
      `页脚的内容宽度不一致：main.ts 的 ${CONTENT_WIDTH}px vs footer.ts 默认的 ${CANVAS_CONTENT_WIDTH}px。` +
        `改 CANVAS_WIDTH/PAD 时请同步 footer.ts 的 CANVAS_CONTENT_WIDTH。`,
    );
  }
}
 ...
 */
const effects: EffectsLayer = mountEffects();
...
(window as any).__gameHome = {
  ...
  carousel: carousel
    ? {
        ...
        goTo: (index: number) => carousel.goTo(index),
  ...
  effects: {
    handle: effects,
    canvasId: 'bg',
    size: () => effects.stats().canvas,
    particles: () => effects.stats().particles,
    frames: () => effects.stats().frames,
    paused: () => effects.stats().paused,
    reducedMotion: () => effects.stats().reducedMotion,
  },
  ...
  footer: { links: footer.links, height: footer.height, coverProgress: footer.coverProgress },
};
