...
/**
 * **整机仿真**（`machines/`）整目录豁免 —— 理由不是"上游同形移植"，而是**判据本身**：
 *
 * 这些机器模拟的是"画布里的一个设备 / 一个操作系统"（街机外壳、XP 桌面），配色是**仿真皮肤**：
 * 外壳的深色塑料、屏幕底、壁纸、图标、窗口皮肤，以及**它们自己那块屏幕上的文字白/灰**
 * （实测 `windows-xp/main.ts` 223 处里 121 处是皮肤常量表与渐变、41 处是自绘屏幕上的文字色）。
 *
 * 把它们改成 `token('ui.colors.*')` **会更糟**：浅色 UI 主题下，街机屏幕的字会变成深色压在深色屏幕上。
 * 判据是"换主题时它该不该变"——**不该变的就不是界面色**。
 *
 * ⚠️ 这不等于机器可以随便写死：机器里的 **ICE 组件**（窗口 / 菜单 / 按钮 / 列表）必须跟着
 * **它自己注册的主题**走（两台机器都是 `registerTheme('xp'|'arcade', …).setTheme(name, ice)`）；
 * 页面级外壳（标题栏 / 说明文字 / 面板底）走本仓统一的主题引用。
 *
 * 也**不做逐文件登记**：单文件 200+ 个仿真色值逐个写理由没有信息量，反而会让人误以为这些是欠债。
 */
